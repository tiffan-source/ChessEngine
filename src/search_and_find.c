...
ScoredMove call_search_algorithm(Game* game, int depth)
{
    ...
    scored_move = nega_alpha_beta_with_move_ordering_and_pv(game, depth, MIN, MAX, &pv);
...
ScoredMove nega_alpha_beta_with_move_ordering_and_pv(Game *game, int depth, int alpha, int beta, PV *pv)
{
    ...
    for (int i = 0; i < moves_list.current_index; i++)
    {
        ...
        if(!is_king_attacked_by_side(&new_game_state, new_game_state.turn)){ // Little hack here Side and TURN are aligned
            move_found = 1;
            scored_move = nega_alpha_beta_with_move_ordering_and_pv(&new_game_state, depth - 1, -beta, -alpha, &child_pv);
