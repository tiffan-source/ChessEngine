...
void scored_move_using_mvv_lva(Game* game, ScoredMove* scored_move)
{
...
void order_move_using_mvv_lva(Game* game, MoveList* move_list)
{
    for (int i = 0; i < move_list->current_index; i++)
    {
        scored_move_using_mvv_lva(game, move_list->moves + i);
    }
